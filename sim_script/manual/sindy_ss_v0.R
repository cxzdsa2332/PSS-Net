...
################################################################################
# sindy_ss_v3.R  —  PSS-Net: 8-species microbial community
...
# ── Edge colour: positive = steelblue, negative = tomato ──────────────────────
edge_col <- function(w) ifelse(w > 0, "steelblue3", "tomato3")
...
ads_ecol <- ifelse(ads_class == "TP",
                   ifelse(ads_w > 0, "steelblue3", "tomato3"),
...
legend("bottomleft", bty = "n", cex = 0.75,
       legend = c("TP positive", "TP negative", "FP", "FN (missed)"),
       col    = c("steelblue3", "tomato3", "orange2", "grey60"),
       lwd    = c(2, 2, 1.2, 1.2),
       lty    = c(1, 1, 2, 3))
