...
| 子图 | 要做什么 | 对应文件 / 对象 | 当前状态 |
|------|----------|-----------------|----------|
| `Fig2a_highdim_sample_complexity` | 展示 node-wise ADSIHT 的网络恢复是否随重标度样本量 `N / (s log p)` 改善；当前维度为 `p = 8, 50, 100`，其中 `p = 8` 与 Fig1 小网络对应。标注理论参考线 `N = 1, 2, 5 x s log(p)` 以及达到 `MCC > 0.8` 所需的插值预算。 | 模拟：`sim_script/02_scaling_design/Fig2a_highdim_sample_complexity.R`。结果：`results/sim_results/Fig2a_highdim_sample_complexity.csv`。绘图对象：`sim_script/02_scaling_design/Fig2.R` 中 `Fig2a_highdim_sample_complexity`、`Fig2a`。 | 绘图初版完成。当前只展示 ADSIHT 的 MCC sample-complexity 曲线，避免与 Fig1c 的方法比较重复；后续主文前建议增加重复数、AUPRC/AUROC 和失败率。 |
| `Fig2b_design_linear` | 在线性稳态系统中比较 random、maximin、D-optimal 扰动设计的信息效率；另用 3D 扰动空间概念图解释三类设计策略的差异。 | 模拟：`sim_script/02_scaling_design/Fig2b_design_linear.R`。结果：`results/sim_results/Fig2b_design_linear.csv`。绘图对象：`sim_script/02_scaling_design/Fig2.R` 中 `Fig2b_design_linear`、`Fig2b`；概念示意：`sim_script/02_scaling_design/Fig2_explain.R` 中 `Fig2_design_concept`。通用绘图仍可用 `analysis_script/plot_design_curves.R`。 | 初版完成。计算脚本由 `pss_net_design.R` 改名而来，固定 `R = 20` 个种子；3D 概念示意已补充；主文前建议增加重复数。 |
